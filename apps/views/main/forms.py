...
class UserConfigForm(Form):
    username = StringField(u'用户名:', validators=[DataRequired(message=u'用户名不能为空!'), Length(min=2, max=6, message=u'用户名长度必须在2-6个字符之间!')])
    password = PasswordField(u'原密码:', validators=[DataRequired(message=u'密码不能为空!')])
    newpassword = PasswordField(u'新密码:',validators=[DataRequired(message=u'您的密码长度必须在6-12个字符之间!')])
    realname = StringField(u'真实姓名:', validators=[DataRequired(Length(max=6, message=u'长度不可以超过8个字'))])
    e_mail = StringField(u'您的邮箱:')
    sex = RadioField(u'性别:', choices=[('man', u'男'), ('woman', u'女')], default='man')
